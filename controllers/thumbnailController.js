...
exports.createThumbnail = async (request, reply) => {
  try {
    const parts = request.parts(); // Correct method for multipart
    let fields = {};
    let filename = null;
    ...
    for await (const part of parts) {
      if (part.file) {
        ...
        fields[part.fieldname] = part.value; // Correct: use part.fieldname
      }
    }
...
exports.deleteThumbnail = async (request, reply) => {
  try {
    ...
    fs.unlink(filepath, (err) => {
      if (err) request.server.log.error(err); // Use request.server.log for Fastify logging
    });
